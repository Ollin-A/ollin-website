...
export default function BlogIndex() {
  ...
  const pageDescription = "Insights, playbooks, and updates on systems that help service businesses grow calmly — and scale deliberately.";
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (error) {
        ...
        setPosts([]);
        setErrorMsg(`${error.code ?? ""} ${error.message ?? "Unknown error"}`.trim());
        setLoading(false);
        return;
      }
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex gap-2 overflow-x-auto pb-1">
          <button
            ...
            className={`inline-flex items-center gap-2 px-3 py-1 text-xs border border-black/10 ${activeTag === "all" ? "bg-black text-white" : "bg-white/50 text-black/70 hover:text-black"}`}
          >
            All
          </button>
          {allTags.map((t) => (
            <button
              ...
              className={`inline-flex items-center gap-2 px-3 py-1 text-xs border border-black/10 ${activeTag === t ? "bg-black text-white" : "bg-white/50 text-black/70 hover:text-black"}`}
            >
              ...
              >
                {p.cover_image_url ? (
                  <div className="relative h-32 w-full overflow-hidden">
                    <img src={p.cover_image_url} className="h-full w-full object-cover opacity-95 group-hover:opacity-100 transition" loading="lazy" alt={p.title || "Post cover"} />
                ...
                <div className="p-5">
                  ...
                  {p.excerpt && (
                    <p className="mt-2 text-sm text-black/65 leading-relaxed">{clampText(p.excerpt)}</p>
                  )}
                  {!!p.tags?.length && (
                  ...
                  <div className="mt-4 text-xs font-medium text-black/70 group-hover:text-black transition">Read →</div>
                </div>
              </Link>
            );
          })}
