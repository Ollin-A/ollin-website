...
import PersonalizedPackageHero from "./PersonalizedPackage_Hero";
import PersonalizedPackagePresets from "./PersonalizedPackage_Presets";
import PersonalizedPackageBuilder from "./PersonalizedPackage_Builder";
import PersonalizedPackageModal from "./PersonalizedPackage_Modal";

/**
 * PersonalizedPackage.tsx
 * - No prices
 * - Presets (Foundation/Build/System/Expansion)
 * - Service builder w/ expandable cards
 * - Sticky "Your Plan" summary panel
 * - "Request Quote" modal that sends plan via email
 *
 * Email delivery:
 * 1) Tries POST /api/plan-request (serverless endpoint)
 * 2) Falls back to mailto:contact@ollin.agency with prefilled body
 */
...
const PersonalizedPackage: React.FC = () => {
        ...
        () => [
            {
                ...
                blurb: "Captures leads, qualifies them, routes requests, and can assist with booking.",
                fields: [
                    {
                        id: "handles",
                    ...
                    },
                    {
                        ...
                        options: [
                            ...
                            { value: "advanced", label: "Advanced (by city/service/urgency)" },
                            { value: "unsure", label: "Not sure yet" },
                        ],
                        defaultValue: "simple",
                    },
            ...
            },
            {
                ...
                blurb: "Build trust fast, collect reviews consistently, and reactivate past customers.",
                fields: [
                    {
                        id: "goal",
                        label: "Primary goal",
            ...
            },
            {
                ...
                blurb: "Maps foundation and visibility improvements so homeowners find you locally.",
                fields: [
                    {
                        id: "locations",
            ...
            },
            {
                id: "websites",
                title: "Websites",
                blurb: "Fast, clean, mobile-first pages built to convert visitors into calls and leads.",
                fields: [
                    {
                        id: "existing_site",
            ...
            },
            {
                ...
                blurb: "Ads focused on booked jobs—built with clean tracking and weekly optimization.",
                fields: [
                    {
                        id: "platforms",
                    ...
                    },
                    {
                        ...
                        helper: "This is NOT our fee. It’s the media budget that goes to Google/Meta.",
            ...
            },
            {
                ...
                blurb: "Consistent posting + trust-building. Simple, clean, and professional presence.",
                fields: [
                    {
                        id: "profiles_setup",
            ...
            },
            {
                ...
                blurb: "See what’s working—calls, forms, and (optionally) ROI attribution from closed jobs.",
                fields: [
                    {
                        id: "goal",
            ...
            },
            {
                ...
                blurb: "A clear diagnosis + 90-day plan. Optional quick-win fixes after the audit.",
                fields: [
                    {
                        id: "why",
            ...
            },
            {
                ...
                blurb: "Clean up your look and trust layer—logo, templates, and polished visuals.",
                fields: [
                    {
                        id: "starting_point",
                        ...
                        options: [
                            ...
                            { value: "brand_kit", label: "Brand kit (mini guidelines + templates)" },
    ...
    // ✅ Presets renamed + icons updated
    const PRESETS: PresetDef[] = useMemo(
        () => [
            {
                id: "foundation",
            ...
            },
            {
                ...
                icon: <Zap size={18} />,
                services: ["websites", "local_seo_maps", "reviews_engine", "analytics_tracking"],
                defaults: {
            ...
            },
            {
                ...
                icon: <Layers size={18} />,
                services: ["digital_secretary", "reviews_engine", "performance_marketing", "analytics_tracking"],
                defaults: {
                    digital_secretary: {
            ...
            },
            {
                ...
                icon: <Crown size={18} />,
                services: ["digital_secretary", "analytics_tracking", "local_seo_maps", "performance_marketing"],
                defaults: {
                    digital_secretary: {
                    ...
                    analytics_tracking: { goal: "roi", locations: "2+", data_quality: "messy" },
                    local_seo_maps: { locations: "2-3", zones: "3+", gbp_status: "messy" },
                    performance_marketing: { coverage: "multi_area", ad_budget: "aggressive" },
                },
            },
        ],
        []
    ...
    const applyPreset = (p: PresetDef) => {
        ...
        setAnswers(() => {
            ...
            for (const sid of p.services) merged[sid] = { ...(p.defaults[sid] || {}) };

            // then apply per-field defaultValue if not set
            for (const sid of p.services) {
                const service = SERVICES.find((s) => s.id === sid);
                if (!service) continue;
                const a = merged[sid] || {};
                for (const f of service.fields) {
                    if (a[f.id] === undefined && f.defaultValue !== undefined) a[f.id] = f.defaultValue;
                }
                merged[sid] = a;
            }

            return merged; // preset means "start here"
        });
    };
    ...
    const setFieldValue = (serviceId: string, fieldId: string, value: any) => {
        setAnswers((prev) => ({
            ...prev,
    ...
    const selectedServices = useMemo(() => {
        const ids = Object.entries(selected)
            .filter(([_, v]) => v)
            .map(([k]) => k);
        return SERVICES.filter((s) => ids.includes(s.id)); // keep SERVICES order
    }, [selected, SERVICES]);

    const preset = useMemo(() => PRESETS.find((p) => p.id === presetId) || null, [PRESETS, presetId]);

    const getOptionLabel = (field: Field, value: any) => {
    ...
    const summarizeService = (service: ServiceDef) => {
        ...
        for (const f of service.fields) {
            ...
            if (f.optional && (v === undefined || v === "" || (Array.isArray(v) && v.length === 0))) continue;
            ...
            if (f.type === "multi") {
                if (Array.isArray(v) && v.length > 0) {
                    const labels = v
                        .map((x) => (f.options ? getOptionLabel(f, x) : String(x)))
                        .filter(Boolean);
            ...
            if (f.type === "text") {
                if (typeof v === "string" && v.trim().length > 0) parts.push(`${f.label}: ${v.trim()}`);
                continue;
            }
    ...
    const completeness = useMemo(computeCompleteness, [selectedServices, answers]);
    ...
    const buildPlainTextSummary = () => {
        ...
        lines.push(`Starting level: ${preset ? preset.title : "Custom / not selected"}`);
    ...
    const submitPlan = async () => {
        ...
        const payload = {
            to: EMAIL_TO,
            lead,
            preset: preset ? { id: preset.id, title: preset.title } : null,
            ...
            answers,
            plainText: buildPlainTextSummary(),
            meta: {
                completeness,
        ...
        try {
            ...
            setSubmitOk(false);
            setSubmitMsg("Couldn’t send automatically. Opening email draft...");
            openMailtoFallback();
        } finally {
            setSubmitting(false);
        }
    ...
    // ---------- UI ----------
    return (
        <main className="w-full min-h-screen bg-ollin-bg text-ollin-black [overflow-x:clip] relative">
            <div className="max-w-[1200px] mx-auto px-5 md:px-8 pt-14 pb-16">
                ...
                <PersonalizedPackageBuilder
                    ...
                    expanded={expanded}
                    answers={answers}
                    preset={preset ? { id: preset.id, title: preset.title, tagline: preset.tagline } : null}
...
export default PersonalizedPackage;
